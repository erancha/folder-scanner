...
/**
 * The on-demand compare path behind --compare=FROM,TO: diffs two stored dated snapshots from
...
public final class SnapshotComparer {
    ...
    /**
     * Reads the from and to snapshots from dir and prints the diff of to against from (the same
     * growth and new-folder sections a scan produces). A requested date with no snapshot file fails
     * with a message naming that date.
     */
