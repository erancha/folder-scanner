 ...
 *
 * since is the pure day-over-day diff behind --baseline: it keeps only folders that grew by strictly
 * more than the threshold. A folder the baseline did not record (or recorded at zero bytes) has no
 * defined growth percentage, so since excludes it; appeared surfaces exactly those folders instead,
 * so a large subtree that materializes overnight is reported rather than dropped.
 */
public record FolderGrowth(Path path, long was, long now, double pct) {
     ...
     * by more than thresholdPct percent. Ranked by growth percent descending, path ascending
     * as the deterministic tiebreaker.
     */
    public static List<FolderGrowth> since(Map<Path, Long> baselineBytes, List<FolderSize> current,
            double thresholdPct) {
        List<FolderGrowth> grown = new ArrayList<>();
        for (FolderSize row : current) {
            Long was = baselineBytes.get(row.path());
            if (was == null || was == 0L)
                continue;
     ...
     * bytes. These have no defined growth percentage and so are excluded from since; reported
     * on their own, ranked by current size descending, path ascending as the deterministic tiebreaker.
     */
    public static List<FolderSize> appeared(Map<Path, Long> baselineBytes, List<FolderSize> current) {
